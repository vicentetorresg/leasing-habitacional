...
export default function FirmaFirmar() {
  ...
  async function load() {
    ...
    try {
      ...
      if (error || !signer) {
        setErrorMsg("Este enlace no es valido o ha expirado.");
        setState("error");
        return;
      }
      ...
      if (signer.signed_at) {
        ...
        setEnvelope(signer.firma_envelopes);
        setState(signer.firma_envelopes.status === "completed" ? "completed" : "signed");
        return;
      }
      ...
      const mappedFields: Field[] = (fieldData || []).map((f: any) => ({
        id: f.id,
      ...
      const signers: Signer[] = (env.firma_signers || []).map((s: any) => ({
        id: s.id,
        name: s.name,
        color: s.color,
      }));
      ...
      setAllSigners(signers);
      setState("ready");
    } catch (e: any) {
      setErrorMsg(e.message || "Error al cargar el documento.");
      setState("error");
    }
  }
  ...
  async function handleSignatureConfirm(dataUrl: string) {
    ...
    const allFieldsSigned = myFields.every((f) => newSigned.includes(f.id));
  ...
  async function submitSignature(dataUrl: string) {
    ...
    try {
      ...
      const res = await fetch(`${CRM_URL}/functions/v1/firma-sign`, {
        method: "POST",
        headers: {
      ...
      if (result.allSigned) {
        setState("completed");
        const { data: updatedEnv } = await supabase
      ...
      alert("Error al enviar la firma: " + e.message);
    } finally {
      setSubmitting(false);
    }
  }
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-md p-10 text-center max-w-sm">
          ...
          <p className="text-gray-500 text-sm mb-4">
            Tu firma fue registrada. El documento se completara cuando todos los firmantes hayan firmado.
          </p>
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-6">
        <div className="bg-white rounded-2xl shadow-md p-10 text-center max-w-sm">
          ...
          <p className="text-gray-500 text-sm mb-6">
            El documento fue firmado por todos. Recibiras el PDF final en tu correo.
          </p>
  ...
  return (
    <div className="min-h-screen bg-gray-100">
        ...
        <div className="max-w-4xl mx-auto px-4 py-3 flex items-center justify-between">
          ...
          <div className="flex items-center gap-3">
            <span className="text-sm text-gray-500 hidden sm:block">
              Hola, <strong>{signerData?.name}</strong>
            </span>
            {allFieldsSigned && submitting && (
            ...
            {!allFieldsSigned && (
              <div className="bg-amber-100 text-amber-700 text-xs font-medium px-3 py-1.5 rounded-full">
                {unsignedFields.length} campo(s) pendiente(s)
              </div>
            )}
          </div>
        </div>
      ...
      {!allFieldsSigned && (
        <div className="bg-[#1B3A6B] text-white text-center text-sm py-2.5 px-4 font-medium">
          Haz clic en el campo resaltado para agregar tu firma
        </div>
      )}

      <div className="max-w-4xl mx-auto px-4 py-6">
        <div className="bg-white rounded-lg border px-4 py-2 mb-4 flex items-center gap-2">
          <span className="text-xs text-gray-400 font-mono">
            Llave Propia Firma ID: {envelope?.id?.replace(/-/g, "").toUpperCase().substring(0, 32)}
          </span>
        </div>
          ...
          <div className="flex flex-wrap gap-2">
            {(envelope?.firma_signers || []).map((s: any) => (
              <div key={s.id} className="flex items-center gap-1.5 bg-gray-50 rounded-full px-3 py-1.5">
                ...
                {(s.signed_at || (s.id === signerData?.id && allFieldsSigned)) && (
