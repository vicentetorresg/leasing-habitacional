export default async function handler(req, res) {
  ...
  // ─── CONFIRM: Photo upload confirmation email ───
  if (action === 'confirm') {
    const { vivienda_id, photo_count } = req.body || {};
    if (!vivienda_id) return res.status(400).json({ error: 'Falta vivienda_id' });

    const vivRes = await fetch(`${CRM_URL}/rest/v1/viviendas?id=eq.${vivienda_id}&select=nombre,email,tipo_vivienda,comuna`, {
...
<table width="100%" style="max-width:520px;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
  ...
  // ─── REQUEST: Ask owner to upload photos ───
  if (action === 'request') {
...
<table width="100%" style="max-width:520px;background:#fff;border-radius:14px;overflow:hidden;box-shadow:0 4px 20px rgba(0,0,0,0.08)">
  ...
  return res.status(400).json({ error: 'action requerido: confirm|request' });
}
