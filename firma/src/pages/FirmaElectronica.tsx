...
import {
  ...
  FileSignature, Clock, CheckCheck, Lock, LogOut
} from "lucide-react";
...
const SESSION_KEY = "firma_lp_auth";

function LoginGate({ onAuth }: { onAuth: (user: string) => void }) {
    ...
    <div className="min-h-screen bg-gray-50 flex items-center justify-center">
      <div className="bg-white rounded-2xl border shadow-sm p-8 w-full max-w-sm">
        <div className="flex flex-col items-center mb-7">
          <div className="w-11 h-11 bg-[#1B3A6B] rounded-xl flex items-center justify-center mb-3">
            <Lock className="w-5 h-5 text-white" />
          </div>
          <h1 className="font-bold text-gray-900 text-xl">Firma Electronica</h1>
          <p className="text-sm text-gray-500 mt-1">Llave Propia · Acceso restringido</p>
        </div>
        <form onSubmit={handleLogin} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Usuario</label>
            <Input value={user} onChange={(e) => { setUser(e.target.value); setError(false); }} placeholder="usuario" className="h-11" autoFocus />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-1.5">Contrasena</label>
            <Input type="password" value={pass} onChange={(e) => { setPass(e.target.value); setError(false); }} placeholder="••••••••" className="h-11" />
          </div>
          {error && <p className="text-sm text-red-500 text-center">Usuario o contrasena incorrectos</p>}
...
export default function FirmaElectronica() {
  const [authedUser, setAuthedUser] = useState<string | null>(() => sessionStorage.getItem(SESSION_KEY));
  const [view, setView] = useState<"list" | "create">("list");

  if (!authedUser) return <LoginGate onAuth={(u) => setAuthedUser(u)} />;
  ...
  if (view === "list") return <EnvelopeList onNew={() => setView("create")} user={authedUser} onLogout={logout} />;
  return <CreateEnvelope onDone={() => setView("list")} />;
}

function EnvelopeList({ onNew, user, onLogout }: { onNew: () => void; user: string; onLogout: () => void }) {
  ...
  async function sendReminder(envelopeId: string) {
    const res = await fetch(`${SUPABASE_URL}/functions/v1/firma-reminder`, {
      method: "POST",
      headers: { "Content-Type": "application/json", apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
      body: JSON.stringify({ envelope_id: envelopeId }),
    });
    const data = await res.json();
    alert(data.error ? `Error: ${data.error}` : `${data.reminded} recordatorio(s) enviado(s)`);
  }

  const statusConfig: Record<string, { label: string; cls: string; icon: React.ReactNode }> = {
    draft: { label: "Borrador", cls: "bg-gray-100 text-gray-600", icon: <FileText className="w-3 h-3" /> },
    sent: { label: "En progreso", cls: "bg-blue-100 text-blue-700", icon: <Clock className="w-3 h-3" /> },
    completed: { label: "Completado", cls: "bg-green-100 text-green-700", icon: <CheckCheck className="w-3 h-3" /> },
  };
        ...
        <div className="max-w-5xl mx-auto px-6 py-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              ...
              <p className="text-xs text-gray-500">Llave Propia · {user}</p>
            </div>
          </div>
          <div className="flex items-center gap-2">
            <Button onClick={onNew} className="gap-2"><Plus className="w-4 h-4" /> Nuevo sobre</Button>
            <button onClick={onLogout} className="text-gray-400 hover:text-gray-600 p-2" title="Cerrar sesion">
      ...
      <div className="max-w-5xl mx-auto px-6 py-8">
        {isLoading ? (
          ...
          <div className="text-center py-24">
            ...
            <Button onClick={onNew} className="gap-2"><Plus className="w-4 h-4" /> Crear primer sobre</Button>
          </div>
        ) : (
                ...
                <div key={env.id} className="bg-white rounded-xl border shadow-sm p-5 hover:shadow-md transition-shadow">
                  <div className="flex items-start gap-4">
                    ...
                    <div className="flex items-center gap-2 flex-shrink-0">
                      <span className="text-sm text-gray-500">{signed}/{signers.length}</span>
...
function CreateEnvelope({ onDone }: { onDone: () => void }) {
  ...
  function addSigner() {
    ...
    setSigners((prev) => [...prev, { id: uid(), name: "", email: "", jobTitle: "", company: "", color: SIGNER_COLORS[prev.length % SIGNER_COLORS.length] }]);
  }
  ...
  function canGoNext() {
    ...
    if (step === 3) return signers.every((s) => fields.some((f) => f.signerId === s.id));
    return true;
  }
  ...
  async function handleSend() {
    ...
    try {
      ...
      const { error: envErr } = await supabase.from("firma_envelopes").insert({
        id: envelopeId, title, pdf_url: urlData.publicUrl, pdf_path: `${envelopeId}/original.pdf`,
        sender_name: senderName, sender_email: senderEmail, status: "draft",
      });
      if (envErr) throw envErr;

      const signerInserts = signers.map((s, i) => ({
        id: s.id, envelope_id: envelopeId, name: s.name, email: s.email,
        job_title: s.jobTitle || null, company: s.company || null, color: s.color, order_num: i + 1,
      }));
      ...
      if (fields.length > 0) {
        const fieldInserts = fields.map((f) => ({
          envelope_id: envelopeId, signer_id: f.signerId, page_index: f.pageIndex,
          x_percent: f.xPercent, y_percent: f.yPercent, width_percent: f.widthPercent, height_percent: f.heightPercent,
        }));
        const { error: fieldsErr } = await supabase.from("firma_fields").insert(fieldInserts);
        if (fieldsErr) throw fieldsErr;
      }

      const res = await fetch(`${SUPABASE_URL}/functions/v1/firma-send-envelope`, {
        method: "POST",
        headers: { "Content-Type": "application/json", apikey: SUPABASE_KEY, Authorization: `Bearer ${SUPABASE_KEY}` },
        body: JSON.stringify({ envelope_id: envelopeId }),
      });
      ...
      alert(`Sobre enviado a ${signers.length} firmante(s)`);
      onDone();
    } catch (e: any) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="max-w-6xl mx-auto px-6 py-8">
                ...
                <div>
                  ...
                  {pdfFile ? (
                    <div className="flex items-center gap-3 p-4 bg-blue-50 border border-blue-200 rounded-xl">
                      ...
                      <button onClick={() => setPdfFile(null)} className="text-gray-400 hover:text-red-500"><Trash2 className="w-4 h-4" /></button>
                    </div>
                  ) : (
                ...
                {signers.map((signer, i) => (
                  <div key={signer.id} className="border rounded-xl p-4">
                    <div className="flex items-center justify-between mb-3">
                      ...
                      {signers.length > 1 && (
                        <button onClick={() => removeSigner(signer.id)} className="text-gray-300 hover:text-red-500 transition-colors"><Trash2 className="w-4 h-4" /></button>
                      )}
                    </div>
        ...
        {step === 3 && (
          <div className="flex gap-6">
              ...
              <div className="bg-white rounded-2xl shadow-sm border p-5 sticky top-6">
                ...
                <div className="space-y-2 mb-5">
                  {signers.map((s) => {
                    ...
                    return (
                      <button key={s.id} onClick={() => setSelectedSignerId(isSelected ? null : s.id)}
                        className={`w-full flex items-center gap-2 p-2.5 rounded-lg border-2 text-left transition-all ${isSelected ? "border-current" : "border-gray-100 hover:border-gray-200"}`}
                        style={isSelected ? { borderColor: s.color, backgroundColor: s.color + "10" } : {}}>
                        <div className="w-3 h-3 rounded-full flex-shrink-0" style={{ backgroundColor: s.color }} />
                        <div className="flex-1 min-w-0"><p className="text-sm font-medium text-gray-800 truncate">{s.name || "Sin nombre"}</p></div>
                        <span className={`text-xs font-medium px-1.5 py-0.5 rounded-full ${fieldCount > 0 ? "bg-green-100 text-green-700" : "bg-gray-100 text-gray-400"}`}>{fieldCount}</span>
                      </button>
                    );
                  })}
                </div>
                {selectedSignerId && (
                  <div className="bg-blue-50 rounded-lg p-3 text-xs text-blue-700">
                    <strong>Arrastra</strong> sobre el PDF para colocar el campo de firma.
                  </div>
                )}
                  ...
                  {signers.map((s) => {
                    ...
                    return (
                      <div key={s.id} className="flex items-center gap-1.5 text-xs">
                        {hasField ? <CheckCircle2 className="w-3.5 h-3.5 text-green-500" /> : <div className="w-3.5 h-3.5 rounded-full border-2 border-gray-300" />}
            ...
            <div className="flex-1 overflow-auto">
              <PDFViewer source={pdfFile!} fields={fields} signers={pdfSigners} onFieldAdd={addField} onFieldRemove={removeField} selectedSignerId={selectedSignerId} />
            </div>
          </div>
        )}
                  ...
                  <div className="w-full">
                    ...
                    {signers.map((s) => (
                      <div key={s.id} className="flex items-center gap-2 py-1.5 border-b border-gray-100 last:border-0">
                        ...
                        <span className="ml-auto text-xs text-gray-400">{fields.filter((f) => f.signerId === s.id).length} campo(s)</span>
                      </div>
                    ))}
                  </div>
                </div>
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-4 text-sm text-amber-800">
                  Al enviar, cada firmante recibira un email con un enlace unico para firmar el documento.
                </div>
              </div>
              <Button className="w-full h-12 gap-2 text-base font-semibold" onClick={handleSend} disabled={sending}>
                {sending ? <><RefreshCw className="w-4 h-4 animate-spin" /> Enviando...</> : <><Send className="w-4 h-4" /> Enviar sobre a {signers.length} firmante(s)</>}
              </Button>
            </div>
          </div>
        )}
        ...
        <div className="flex justify-between mt-8 max-w-6xl">
          ...
          {step < 4 && (
            <Button disabled={!canGoNext()} onClick={() => setStep((s) => Math.min(4, s + 1) as Step)} className="gap-2">
