...
export default function SignatureDrawer({ signerName, onConfirm, onCancel }: SignatureDrawerProps) {
  ...
  async function renderAutoSignature() {
    ...
    try { await document.fonts.load(`48px "${fontFamily}"`); } catch (_) {}
    ...
    ctx.strokeStyle = "#d1d5db"; ctx.lineWidth = 1;
    ctx.beginPath(); ctx.moveTo(40, canvas.height - 28); ctx.lineTo(canvas.width - 40, canvas.height - 28); ctx.stroke();
  }
        ...
        <div className="p-6">
          <Tabs value={activeTab} onValueChange={setActiveTab}>
            <TabsList className="w-full mb-5">
              <TabsTrigger value="draw" className="flex-1 gap-2"><PenLine className="w-4 h-4" /> Dibujar</TabsTrigger>
              <TabsTrigger value="auto" className="flex-1 gap-2"><Type className="w-4 h-4" /> Nombre</TabsTrigger>
            </TabsList>
            <TabsContent value="draw" className="mt-0">
              ...
              <div className="relative border-2 border-gray-200 rounded-xl overflow-hidden cursor-crosshair bg-white">
                <canvas ref={drawCanvasRef} width={560} height={200} className="w-full touch-none"
              ...
              {hasDrawn && (
                <button onClick={clearDraw} className="flex items-center gap-1.5 text-sm text-gray-400 hover:text-red-500 mt-2 transition-colors">
            ...
            <TabsContent value="auto" className="mt-0">
              ...
              <div className="border-2 border-gray-200 rounded-xl overflow-hidden bg-white">
                <canvas ref={autoCanvasRef} width={560} height={200} className="w-full" />
              </div>
            </TabsContent>
          </Tabs>
          <div className="mt-4 bg-gray-50 rounded-lg px-3 py-2 flex items-center gap-2">
            <div className="w-2 h-2 bg-green-500 rounded-full flex-shrink-0" />
            <p className="text-xs text-gray-500">Tu firma llevara un ID unico de verificacion para dar validez legal al documento.</p>
          </div>
        </div>
        <div className="px-6 pb-6 flex gap-3">
          <Button variant="outline" className="flex-1" onClick={onCancel}>Cancelar</Button>
          <Button className="flex-1" disabled={!canConfirm} onClick={handleConfirm}>Confirmar firma</Button>
        </div>
      </div>
    </div>
  );
