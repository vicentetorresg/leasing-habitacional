...
var HEADERS = ['Fecha/Hora', 'Nombre', 'Telefono', 'Email', 'Arriendo', 'Fuente', 'Ahorro', 'Dicom', 'Renta', 'Contrato Indefinido'];
...
function syncLeads() {
  ...
  if (sheet.getLastRow() === 0) {
    ...
    sheet.getRange(1, 1, 1, HEADERS.length).setBackground('#1B3A6B').setFontColor('#FFFFFF').setFontWeight('bold');
  ...
  for (i = 0; i < rows.length; i++) {
    ...
    sheet.appendRow([ts, r.nombre || '', r.telefono || '', r.email || '', r.arriendo || '', r.fuente || '', r.ahorro || '', r.dicom || '', r.renta || '', r.contrato || '']);
  }

  props.setProperty('last_sync', rows[rows.length - 1].created_at);
